...
11. `docs/COMFY_CLOUD_API_CONNECTION_GUIDE.md`
   - Comfy Cloud API를 비용 없이 읽기 전용으로 먼저 확인하는 방법을 설명합니다.
...
## 현재 실행 가능한 Comfy Cloud 읽기 전용 연결 확인

```powershell
& '.\.tools\python312\python.exe' -m tools.check_comfy_cloud_connection
```

이 명령은 `COMFY_CLOUD_API_KEY` 환경 변수를 사용해 서버 상태만 확인합니다. 생성 작업을 제출하거나 크레딧을 사용하지 않습니다.
